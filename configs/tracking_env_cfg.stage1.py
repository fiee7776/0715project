...
class CommandsCfg:
    ...
    motion = mdp.MotionCommandCfg(
        ...
        motion_file="/root/gpufree-data/gmr_outputs/input5_v2_50hz.npz",
        ...
        velocity_range={k: (0.0, 0.0) for k in VELOCITY_RANGE},
...
class EventCfg:
    ...
    # startup
    # Stage 1: disable friction randomization so the policy first learns the video motion.
    physics_material = None

    add_joint_default_pos = None

    # Stage 1: disable torso COM randomization; re-enable after tracking is stable.
    base_com = None

    # interval
    # Stage 1: disable random pushes; re-enable for sim-to-real robustness later.
    push_robot = None
...
class RewardsCfg:
    ...
    joint_torque = RewTerm(func=mdp.joint_torques_l2, weight=-5e-6)
...
class TerminationsCfg:
    ...
    # Stage 1: do not terminate immediately when hands/feet are far from the video.
    # The body tracking rewards still teach these links; re-enable this after the
    # policy can follow the whole clip.
    ee_body_pos = None
...
class RobotPlayEnvCfg(RobotEnvCfg):
    def __post_init__(self):
        ...
        self.commands.motion.fixed_start_time_step = 0
