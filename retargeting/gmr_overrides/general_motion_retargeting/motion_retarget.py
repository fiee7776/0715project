...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        self.joint_posture_costs = ik_config.get("joint_posture_costs", {})
    ...
    def setup_retarget_configuration(self):
        ...
        if self.joint_posture_costs:
            posture_cost = np.zeros(self.model.nv)
            for joint_name, cost in self.joint_posture_costs.items():
                if joint_name not in self.robot_dof_names:
                    print(f"[GMR] Warning: joint posture target '{joint_name}' not found in robot model")
                    continue
                posture_cost[self.robot_dof_names[joint_name]] = float(cost)

            if np.any(posture_cost > 0):
                self.joint_posture_task = mink.PostureTask(
                    self.model,
                    cost=posture_cost,
                    lm_damping=1,
                )
                self.joint_posture_task.set_target_from_configuration(self.configuration)
                self.tasks2.append(self.joint_posture_task)
                self.task_errors2[self.joint_posture_task] = []
